    ...
    for (const group of groups) {
      ...
      html += group.workplaces.map(wp => {
        ...
        if (wp.exists && wp.complete) {
          ...
          icon = '✓';
          clickAction = `onclick="event.stopPropagation();if(confirm('Табель рабочего места ${escapeHtml(wp.workplace)} смены «${escapeHtml(group.shift)}» уже заполнен экспертом ${escapeHtml(wp.timesheet.inspector)}. Точно хотите изменить его?'))window.location.href='/create.html?edit=${encodeURIComponent(wp.filename)}'"`;
        } else if (wp.exists && !wp.complete) {
          statusClass = 'status-incomplete';
          icon = '✎';
          clickAction = `onclick="event.stopPropagation();window.location.href='/create.html?edit=${encodeURIComponent(wp.filename)}'"`;
        }
